...
// POST /api/simpan/data - Save or update server data (using JSON body)
serverDataRouter.post("/simpan/data", apiKeyAuth, async (c) => {
  try {
    ...
    if (!ip || !username || !password) {
      return c.json({
        success: false,
        message: "Body ip, username, dan password diperlukan",
      }, 400);
    }
...
serverDataRouter.post("/list/data", apiKeyAuth, async (c) => {
  try {
